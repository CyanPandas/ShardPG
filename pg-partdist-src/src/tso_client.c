 ...
 * 基线断言不动。
 *
 * ★★ 2026-09-12：**试过改成返回 0，是错的，已回退。** 如实留痕以免重犯。
 *   起因是 T7.12-P3 定因 `promote_catchup_tx3`（新主字节全到、applied 追平，
 *   40 行却读不到），我判断病根是"commit_ts 双宇宙"：可见性判据 §4.1 是
 *   `commit_ts < 读者 start_ts`，墙钟值（实测标记里 2114118804）恒大于任何
 *   TSO 号，行应当永久不可见；且 `shard_visibility.c` 写着「commit_ts 给 0 ——
 *   遗留语义正是"对一切快照可见"」，同文件 `PartDistTsoDtxDecisionTs()` 未配置
 *   即返回 0 并注明"遗留宇宙不混"。看起来证据充分。
 *
 *   **实测否证**：改成 0 之后 `promote_catchup_tx3` **依旧 21/2**（说明那不是
 *   它的病根），而 `dtx_commit_marker_tx2` 从 39/2 **恶化到 33/6** ——
 *   它有一条断言就叫「顶层事务 commit_ts 非 0」，被我直接打破。
 *   上面那句"tx1/tx2/tx4 基线断言不动"的警告是对的；我关于"那几套都配了 TSO、
 *   碰不到这一支"的推断是错的。
 *
