...
| b | **快路径分叉的归队规则**（§9.5） | ✅ **2026-08-08 完成**。检测 + 升主闸见 §12.5。验收 `tests/test_fastpath_divergence_tx4.sh` **15/15** |
...
⇒ **§10 的第 0–6 步已全部完成。** 已知未做的只剩性能优化项：Prepare 阶段的
**组间并行**，见 §12.4。

**读本文的顺序**：§0 概括 → §3 协议（正常路径/快路径）→ §5 记录格式 →
§6 状态机 → §7 恢复 → **§9 索引表**（哪些还没解决）→ §10 落地顺序。
§2 是对用户原始方案的四处修正，§4 是协调组选取规则。

> ⏪ **2026-08-07：数据组日志外部化（raft 计划 §11.10 的 E1–E4）整条线已回滚** ——
> 不只是未开工的 E4，**已落地的 E1/E2/E3 也全部 `git revert`**，代码回到外部化开工
> 之前的形态。回滚范围、运行期影响与验证见 **raft 计划 §0.4**。
> 对本文的影响有两条：
> - **§9.9 退回"未解决"** —— `partdist.raft_log` 双写恢复，"raft 日志 SQL 行与用户
>   事务同命"这条隐患回到 E3 之前的状态（该节标题已改回）。
> - §10.1 第 3 条从"已完成 E1–E3、剩 E4"改为"整条未落地"。
>
> 本文其余全部内容（协议、记录格式、状态机、恢复、第 0–5 步的验收）**不受影响** ——
> 2PC 从不依赖日志外部化。

> 📦 **合并 `shardpg-replay`（物理回放线）前必读**：合并的必炸清单、冲突文件处理原则、
> 验收门槛在 **raft 计划 §15**。与本文直接相关的一条是 §15.2 **#1**：
> replay 侧的 `PARTWAL_FLAG_CLASS_MASK = 0x07` **不含** 本线新增的
> `PARTWAL_FLAG_DTX = 0x08`，机械合并会让 DTX 记录被判成 DATA 并喂进 `rm_redo`。
> （它原本排在 #2；排在它前面的那条已随外部化回滚而消失，**现在它是清单里
> 唯一"确定会坏"的一条**。）
...
> 本节标题此前**丢失**（内容一直孤零零挂在 §3.5 末尾，导致目录里从 §3 直接跳到 §5），
> 2026-08-06 整理时补回。规则本身未改。
...
> **本节的小节编号是按发现顺序追加的，不是按严重度排列**（9.2.5 / 9.2.6 排在 9.0
> 前面就是这么来的）。下面这张索引表是唯一的权威顺序，正文一律原地保留 —— 每条
> 的根因分析与踩坑记录都是后续决策的依据。
...
| 小节 | 问题 | 解决时间 / 验收 |
|---|---|---|
| §9.0 | 并发写入路径三缺陷（reader UAF / NULL 行解引用 / leader 截断丢数据） | 2026-08-03，raft_17 |
| §9.1 | **group-commit 让路窗口**：从性能弱化升级为正确性 bug | 2026-08-03，raft_17 三阶段 |
| §9.2 | 成员集未知时被当作"全体节点" ⇒ 多数派算错 | 2026-08-03，raft_18 |
| §9.2.5 | prepared 事务的行锁把决议路径锁死 | 2026-08-03 |
| §9.2.6 | DTX 记录被写进段号 1，排到了它所标记的数据前面 | 2026-08-04 |
| §9.3 | master 侧 decide 的挂点（内核补丁 0004） | 2026-08-04，raft_22 |
| §9.4 | Citus 自带 2PC 恢复会与我们打架 | 2026-08-04，raft_22 A |
| §9.6 | 升主 in-doubt 闭合**机制** | 2026-08-04，raft_23（**接线仍缺，见下**） |
| §9.7 | 容量与 GC（acked 回执 + `DTX_FORGET` + `pg_dist_transaction` GC） | 2026-08-04，raft_20 F / raft_21 I |

**尚未解决**

| 小节 | 问题 | 性质 | 卡在哪 |
|---|---|---|---|
| **§9.5** | **快路径的提交点定义**：单分区事务沿用 `[A]→quorum→[B]` 时序，leader 在 `[B]` 前崩溃 ⇒ 组内认为已提交、本地却中止 ⇒ leader 与自己的组分叉 | **正确性**（故障路径） | 已选定方案 (b)（接受窗口，归队时对分叉分片强制重做物理基线），但**归队规则未实装**，要与惰性回放 promotion 路径合流 |
| **§9.6 接线** | in-doubt 闭合函数 `dtx_close_indoubt()` 已有且单测通过，但**没有插进升主序列** | 正确性 | 同上，等 promotion 路径 |
| §9.7.1 | 一笔跨分区事务跑三轮复制，落后 follower 窗口变宽 | 性能 / 判据时序敏感 | 已由后台追平通道（raft_24）大幅缓解；阶段 3 标记本就是 best-effort |
| §9.8 | 锁滞留（prepared 持锁到决议为止）与 **TSO 缺位** | 可用性 / 全局快照 | 恢复守护超时参数化、`pg_prepared_xacts` 年龄监控待做；TSO 等 R3 立项 |
| §9.9 | raft 日志 SQL 行与用户事务同命 | 窄窗口 | ⏪ **2026-08-06 曾随外部化 E3 标为"基本消解"，2026-08-07 随整条回滚退回未解决**（raft 计划 §0.4）。`raft_log` 双写恢复；短期靠 follower 落盘幂等 + 增量下界容忍，根治仍需外部化 |

> **一句话**：2PC 本体（§10 第 0–5 步）已经全部落地并验收，**剩下的全部集中在第 6 步**
> —— 升主/归队路径。而第 6 步的两件事都不是 2PC 内部问题，是它与惰性回放合流的问题。
...
### 9.1 【已修复】group-commit 让路窗口：从性能弱化升级为正确性 bug（原【最高】）
...
### 9.5 【未解决】快路径的提交点定义需要拍板（方案已选 (b)，规则未实装）
...
> **★ 2026-09-09 复核**：(a) 仍未实装（设计如此）；**归队规则做了一半** ——
> 分叉的**检测**已落地（`promote_prepare` 的分叉分支 `replay_disable` + WARNING、
> `diverged` 标记），**重做基线仍全靠人工**（`fastpath_divergence_tx4` 只验到
> 拒升主为止，没有"重做基线 → 重新参选"的闭环）。收口见
> `P7_REMEDIATION_PLAN.md` T7.14。
...
> 判据是"流里没有提交标记"，有决议的必须先落终局，否则会被误判 ABORTED）。
> **但四级落空时无告警面**（本文 §9.6 末尾自陈），而 R-P6-15 恰好制造了一种
> 必然四级落空的情形：2PC 的 COMMIT 标记不带分片 xid ⇒ 副本 clog 永远 PREPARED
> ⇒ 决议被 FORGET 回收后 `dtx_close_indoubt` 返回 0，**已提交的行在新主上永久
> 不可见**。见 `P7_REMEDIATION_PLAN.md` T7.1。
...
### 9.9 【中｜未解决】raft 日志 SQL 行与用户事务同命

计划 §14.3 #2。DECISION 不受影响（`dtx_decide` 在独立事务里）；
PREPARE 标记在用户事务内 propose 仍有窄窗口，与现状同级风险。
短期靠 follower 落盘幂等 + 增量下界容忍，根治靠日志外部化。

> ⏪ **2026-08-07 状态回退**：本条曾在 2026-08-06 标为"随外部化 E3 基本消解"——
> 那是因为 E3 让数据组的 `OP_PARWAL` 条目不再写 `partdist.raft_log` 行。
> **外部化已整条回滚**（raft 计划 §0.4），`raft_log` 双写恢复，
> 因此本条**退回未解决**，处置方式回到"短期容忍"：propose 在 PRE_COMMIT 内经
> SPI 写 `partdist.raft_log`，若事务在 propose 之后仍中止（后续回调 ERROR 等窄窗口），
> leader 的 SQL 日志行随之回滚而 shmem 环仍在。
> 兜底仍是 follower 落盘幂等 + 增量下界容忍；根治仍需日志外部化，而那条线现已回滚。
...
3. **计划 §12.4 #3 日志外部化** —— ⏪ **整条未落地（2026-08-07 回滚）**。
   E1–E3 曾于 2026-08-05/06 落地（段式边界表 `raft_log_runs` 承载 (term, plsn) 映射、
   读写两条路径切到 parwal 段文件、日志末端随 HardState v4 持久化），
   已按用户指令整条 `git revert`，**现状等同从未做过**，回归基线 59 → 56。
   见 raft 计划 §0.4。连带影响：§9.9 退回未解决；§9.7 那条"决议被日志截断后误判
   推定中止"的窗口回到 E3 之前的宽度。
   **这一条与 2PC 无关**，回滚不影响本文任何协议结论。
   InstallSnapshot 控制面部分随 raft_25 落地，**未被回滚**。
...
> **2026-08-06 复核**：raft 计划那一组已全部处理（该文现有 §0 状态速览可直接对照）；
> FRD 那一组**仍未处理**，且它正是第 6 步合流的前提 —— 下面逐条标了状态。
...
**`pg-raft-src/docs/raft_module_revision_plan.md` —— ✅ 均已处理**
（§0 速览、§2.3 修订痕迹、§8 风险表、§11.10 外部化设计都已同步到最新状态）。

**`pg-partdist-src/docs/FOLLOWER_REPLAY_DESIGN.md` —— ⚠️ 三条全部未处理**，
且这三条正是第 6 步合流的前提：

| | 需要改什么 | 状态 |
|---|---|---|
| a | §4.3 `TxnMarkerPayload` 与本文 §5.2 的 `DtxRecordPayload` 对齐 —— 标记记录要能携带 `dtxid`，PREPARE 是新增的标记类型 | ❌ 未做 |
| b | §7.6 现在只处理 COMMIT/ABORT，须补 in-doubt（PREPARE 已见、决议未见）的处理；§9.3 枚举里现成的 `TXN_PREPARED` 正好用上 | ❌ 未做 |
| c | §11 升主六步：插入 §9.6 的"清 in-doubt"步骤，以及 §9.5 快路径分叉的归队规则 | ❌ 未做，**这就是第 6 步本身** |

> 另有一条独立的前置：**两份 FRD 已经分叉**（临时环境 16KB 旧版 vs 主仓库 28KB v2），
> raft 计划 §12.4 #1 要求 P3 开工前先合并，否则上面三条改在哪一份都说不清。

#### 11.1 合并 `shardpg-replay` 之后，上表要重新核对（2026-08-07）

已核对两个远端分支尖端：

- **FRD 应当取 replay 侧那份**。4.0 侧 1143 行、replay 侧 **1680 行（v3.1）**，
  后者是前者的演进超集（R2 事务层、D1 文件集合通道、D2 freeze 同步都写进去了）。
  合并时 FRD 直接取 replay 版本，**不要**试图把 4.0 侧的旧版内容合回去。
- **上表 a/b/c 三条在 replay 侧 FRD 里依然全部未处理** —— 实测：全文搜
  `in-doubt` / `dtxid` / `DTX` **零命中**，只有 §9.3 枚举里现成的 `TXN_PREPARED = 1`。
  也就是说合并**不会**顺带解决第 6 步，三条改动仍要在合并后的 FRD 上补。
- 具体改在哪：a 改 FRD §4（parwal-3.0 记录格式）、b 改 FRD §7（Follower 回放管线）、
  c 改 FRD **§11 Failover: Follower 提升为 Leader (R4)** —— 这一节就是第 6 步要接线的地方。
- **多出一条新的前置**：replay 侧 R2 把记录头升到 parwal-3.0（`gxid` 取代 `xid`），
...
### 12.5 快路径分叉的归队规则（第 6 步 b，已完成）

§9.5 定的方案 (b)："接受该窗口，规定旧 leader 崩溃后归队时，对检测到状态分叉的
分片强制重做物理基线。"本节记它实际怎么落的。

**检测判据：两份凭据互相矛盾，不比页面。**

段流里的 COMMIT 标记是"**组**认为这笔事务提交了"的凭据；本地 CLOG 是"**本节点**
认为它提交了没有"的凭据。同一个 xid 上两者给出相反答案，就是分叉的直接证据 ——
这正是 `[A] → quorum → [B]` 窗口崩溃后留下的指纹，不需要与新 leader 逐页比对。

```
partdist.pg_raft_check_fastpath_divergence(loid, max_scan) → 分叉条数
```

三条实现要点：

- **只查本节点写的标记**（`gxid >> 48` == 本节点 Citus group id）。别的节点写的
  标记，其 xid 在本地 CLOG 里根本没有意义。
- **从流尾往回扫，遇到第一个"本地确实提交了"的标记就停**。那笔事务的 [B] 已经
  完成，比它更早的不可能停在 [A]-[B] 之间。所以扫描量与分叉深度同阶，正常情况
  第一条就停；`max_scan` 只是防御性上限。
- **`pg_xact_status` 接的是 `xid8`（64 位 full xid），而 gxid 低位存的是 32 位
  `TransactionId`** —— 必须按当前 epoch 还原（`cur - cur % 2^32 + lo`，若结果
  大于当前值则减一个 epoch），硬转会在 epoch 边界上查到隔了一整个 epoch 的
  另一笔事务的判决。（第一版就是硬转，被 TX4 当场抓出来。）

**闸设在升主前置上，不设在归队上。**

惰性回放让这件事变简单了：副本平时一条 redo 都不做，内容**只在升主那一刻**才有
意义。所以分叉的旧 leader 只要永不被提升为主，就不会把分叉数据对外服务 ——
不必在归队时立刻做基线，也不必阻塞它继续接收字节。

因此 `pg_raft_promote_prepare` 的返回值从布尔改成**三态**：

| 返回 | 含义 | 是否可被 deadline 兜底放行 |
|---|---|---|
| `1` | 可以上报 | — |
| `0` | 还没追平，稍后重试 | **是**（可用性优先，见 §12.1） |
| `-1` | **检测到分叉** | **否，永不放行** |

三态而不是布尔，要害就在最后一行：`0` 是暂时状态，可以为了可用性放行；`-1` 是
已知事实，放行等于让一个已知与组分叉的副本当上主库，比"该分片暂时无主"坏得多。
C 侧 `data_group_promote_prepare` 对 `-1` 直接清掉截止期计时并返回，绕过兜底逻辑。

分叉检查排在 `armed` 判断**之前**：分叉的旧 leader 上该分片是真表而不是副本壳表，
多半压根没有回放槽位，先判 `armed` 会直接放行、让检查形同虚设。检出后顺手把回放
槽位下电（若有），避免它以"看似正常"的姿态继续参与。

**验收** `tests/test_fastpath_divergence_tx4.sh`（15/15）。分叉状态是**确定性构造**
的，不靠掐时机崩溃：阶段 3 的钩子跑在 `standard_ProcessUtility` **之前**，所以在
事务块里发 `COMMIT PREPARED` 会让 COMMIT 标记先落盘、语句再被
`PreventInTransactionBlock` 拒掉，prepared 事务原地不动 ⇒ `pg_xact_status`
= `in progress` ≠ `committed` ⇒ 正是要检测的状态。

用例里有两条断言值得单独说：
- **健康分片必须返回 0**（无误报）。一个会误报的检测器等于把所有分片永久禁止升主，
  比不检测更糟，所以这是全部后续断言的前提。
- **`ROLLBACK PREPARED` 之后仍判为分叉**。流里那条 COMMIT 标记还在，组仍然认为
  它提交了，而本地已变成 `aborted` —— 矛盾没有消失。这是**正确**的：分叉是既成
  事实，清掉 prepared 事务并不能让它消失，只有重做物理基线才行。

**剩余边界**：自动化的物理基线重做（由新 leader 经 `log_newpage_range` 把全量内容
