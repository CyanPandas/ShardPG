...
代码基线：`shardpg-4.0` @ `6181655`。回归：`run-raft-tests.sh` **59/59** + P0 **10/10**。
...
> 🧊 **2026-08-07：数据组日志外部化（raft 计划 §11.10 的 E1–E4）整条线冻结** ——
> 不只是未开工的 E4，**已落地的 E1/E2/E3 同样冻结**（保持原样、不回滚、不继续优化）。
> 冻结范围与"允许/不允许做什么"见 **raft 计划 §0.4**。
> 对本文的影响：§9.9 的结论（raft 日志 SQL 行不再与用户事务同命）**继续有效** ——
> 它依赖的是 E3 已落地的事实，不依赖 E4；§10.1 第 3 条中"剩 E4"的待办随之作废。
> 本文其余全部内容不受冻结影响。

> 📦 **合并 `shardpg-replay`（物理回放线）前必读**：合并的必炸清单、冲突文件处理原则、
> 验收门槛在 **raft 计划 §15**。与本文直接相关的一条是 §15.2 #2：
...
| §9.9 | raft 日志 SQL 行与用户事务同命 | 窄窗口 | **随日志外部化 E3 基本消解**：数据组的 OP_PARWAL 条目已不再写 `raft_log` 行（raft 计划 §11.10） |
...
### 9.9 【基本消解】raft 日志 SQL 行与用户事务同命（随外部化 E3）🧊 **就此定格**

计划 §14.3 #2。DECISION 不受影响（`dtx_decide` 在独立事务里）；
PREPARE 标记在用户事务内 propose 仍有窄窗口，与现状同级风险。
短期靠 follower 落盘幂等 + 增量下界容忍，根治靠日志外部化。

> **2026-08-07 定格**：E3 已落地 —— 数据组的 `OP_PARWAL` 条目不再写
> `partdist.raft_log` 行，本条隐患的主体随之消失。外部化整条线现已冻结
> （raft 计划 §0.4），因此**本条的状态就停在"基本消解"，不再往下推**。
> 残留的窄窗口（用户事务内 propose 的 PREPARE 标记）由 follower 落盘幂等
> 与增量下界容忍兜住，属于可接受的既有风险，不作为待办。
...
3. ~~**计划 §12.4 #3 日志外部化**~~ —— **2026-08-05/06 已完成 E1–E3**
   （raft 计划 §11.10，回归 raft_26/27/28）：段式边界表 `raft_log_runs` 承载
   (term, plsn) 映射，读写两条路径都切到 parwal 段文件，日志末端随 HardState v4
   持久化。**数据组的 OP_PARWAL 条目已不再写 `raft_log` 行** ⇒ §9.9 基本消解，
   §9.7 那条"决议被日志截断后误判推定中止"的窗口也随之收窄。
   ~~剩 E4~~ —— 🧊 **2026-08-07：整条外部化线（含已落地的 E1–E3）冻结**
   （raft 计划 §0.4）。E4（数据组不再分配 ring、抬 `RAFT_MAX_GROUPS`）**不开工**，
   它是内存布局问题、与 2PC 无关，冻结不影响本文任何结论。
   InstallSnapshot 控制面部分已随 raft_25 落地。
