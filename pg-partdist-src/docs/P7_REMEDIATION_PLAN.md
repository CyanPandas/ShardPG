...
- **P7-N21（可运维）**：清理顺序颠倒时 DROP TABLE 不可中断热自旋，非数据正确性问题，已有规避（先表后组 + statement_timeout）。
...
**最新一轮结果：PASS=73 / FAIL=1**——8 项已知/环境项降级为"只报"（6 项为 2 vCPU 漂移致副本 0 块跳过、
1 项 N20、1 项 N19），唯一真 FAIL 为 [9] 第 2 轮新主登记在窗口内未取到值（该轮与 N21 的 56 分钟满载
自旋 backend 同机竞争 2 vCPU）。
