...
| **P7-N31** | **切主登记生效的一瞬，协调者路由已翻到新主、新主自己那份登记还没 apply ⇒ 读闸门仍当它是副本、拒读** | 各节点独立 apply 同一条 group0 登记，协调者先 apply 就先改 pg_dist_placement；新主的"已接管为主、解除副本读闸门"要等它自己 apply 到这条 | N25 回归（2026-09-18 18:42:48）：协调者读 rn25 被新主 :5433 拒（`不允许在本节点上对副本壳表（OID 202056）执行查询`），同批 apply 在其后 ~0.3 s | **P2（可用性，亚秒级读失败）** → **未修，仅登记**。方向：新主在升主前置返回 1 时即置已升主（登记被栅栏挡下时由 N27 的标记降级兜回），或读闸门认 raft leader + 在册登记 |
