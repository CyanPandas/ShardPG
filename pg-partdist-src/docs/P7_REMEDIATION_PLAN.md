...
| **P7-N39** | **控制面（0 号组）停在上一任期的尾巴上，一直不提交也不 apply**：Raft 不允许 leader 直接提交前任任期的条目，必须先提交一条本任期的条目把它们带上。数据组在升主前置里做了（P7-N35），**控制面没有人做** | 换任期之后如果控制面一直没有新写入，上一任期那条就永远卡着。它可能正是一条 `OP_PARTITION_PRIMARY`（「谁是这个分片的主」）⇒ 各节点的 `partition_map` 与路由停在旧值；后续控制面写入还会排在它后面 | 2026-09-23 加第 5 个节点时撞上：0 号组停在 `last=1154 / commit=1153`（1154 是任期 677 的 `OP_PARTITION_PRIMARY`，leader 已在任期 678），`citus_add_node` 挂死 5 分钟以上；手工 `pg_raft_group_propose(0,'OP_NOOP','{}')` 之后 commit 立刻推到 1155、5 个节点全部 applied | **P1（控制面停滞 + 路由停在旧值）** → **✅ 已修（2026-09-23）**：控制面 leader 在 tick 里发现「尾巴的任期 < 本任期」就经本机 libpq 异步补提一条 `OP_NOOP`（tick 里不能做 SPI，与登记/升主前置同源），失败下个 tick 重来（5 s 限频）；`OP_NOOP` 在 apply 侧加了正经分支，不再每条打一行「应用失败」。**实测自愈**：整簇重启后日志 `控制面有上一任期(702)的尾巴未提交（idx 1225 > commit 1224，本任期 704），补提一条本任期空条目` → 2.5 s 后 `控制面本任期空条目已提交（idx=1226）`。**回归** `test_ctrl_noop_tail_p7n39.sh`（4 轮整簇重启，每轮断言 30 s 内 0 号组在全部节点 commit == last、任期/提交点收敛一致，9/0） |
...
- **N39 ✅ 已修（2026-09-23）**：控制面缺「本任期空条目」，换任期后上一任期的尾巴永久不提交（实测把 `citus_add_node` 挂死 5 分钟）；与 N35 同族，这次补在 C 侧的 tick 里，带回归 9/0。
