...
### 1.9 2026-09-13 批次 6 施工中新登记的缺陷（W2/W3/W5/T6/T7/P2 已修，W4 需专项）
...
| **P7-T8**<br>（2026-09-13 新登记） | **`txn_layer_r2` 的"丢提案额度"与实际恒差 1**：用例 [10] 故意停掉两个 follower 拆多数派，`health_check_no_drops 2` 给了 2 次额度，实测 `:5433` 恒丢 **3** 次（干净集群、多轮稳定）。原因未定：可能是额度按 T7.27 背压之前的提案次数标定，或拆多数派期间多了一次 CTRL 重试 | **未修，仅登记**。**A/B 已证明先于 T7.29 存在**：不含 G4 的 HEAD（`5268690`）上同一套 49/3、签名同为 `:5433+3`；含 G4 为 51/1。判定前要先弄清第 3 次丢弃是哪条记录（`pg_raft_group_flow_stats` 按记录类型拆分），再决定是改额度还是改拆多数派的时序 —— 不许直接把额度调成 3 了事 |
