...
hardened 跑），N16 空壳主由 baseline_pending 前置挡下。**唯一剩项 = P7-N13**（回滚判决未权威到达升主的
新主 ⇒ 偶发 1 笔 in-doubt 保持 PREPARED，安全但让总额/残留断言偶红）——深层 2PC 决议 durability，
独立立项；验收里把"回滚+连续快切"下的单笔 in-doubt 标为已知残留，不作门禁判绿项。
新增全方位套件 `test_full_mixed_multiprimary_p7.sh`：打标+原生分片并存的多主多从拓扑上跑分布式事务
（commit/rollback/中途失败原子）+ 受控切主（守恒/无重号/副本重追平/逐字节）。
