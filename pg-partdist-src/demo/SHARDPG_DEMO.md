...
3. **（已于 2026-09-20 修复 → P7-N33）切主的一瞬，经协调者的读可能被拒 1–2 s。** 同一条控制面登记由各节点各自应用，先后差 0.2–1.9 s：旧主先应用就先把读闸门合上，而协调者的路由还指向它，这 1–2 s 里的读会报「不允许在本节点上对副本壳表…执行查询」。**修法**：刚降级、且本地还没回放过新主的流时放行读（这一格就是它交出主权那一刻的已提交状态），一旦回放推进或宽限 `pg_partdist.demoted_read_grace_ms`（默认 10 s）到期即恢复拒读。回归 `tests/test_demoted_read_grace_p7n33.sh`（含负向对照：宽限=0 时同一窗口 20 次拒读）。注意这只保「降级 → 被新主重新供成副本」这一段：副本流一到、本地回放一推进，宽限按设计失效——那之后它的页正在被新基线覆盖，本来就不能读。
