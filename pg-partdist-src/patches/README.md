...
| 10 | `0010-promote-advance-wal-insert-position.patch` | `XLogRequestInsertPositionAdvance()` + `XLogAdvancePendingInsertPosition()`（改 `xlog.c`、`xlog.h`） | TX-TSO-MVCC P6（T6.3b）/ FRD §11 步骤 4：**在线**把本地 WAL 插入位点推进到目标 LSN 之后的段边界，语义类似 `pg_resetwal -l`。**跳跃发生在 `CreateCheckPoint` 持有全部插入锁的临界区内**（排在"空闲跳过检查"之后），跳完紧接着算出的 `checkPoint.redo` 就落在新段里 —— 因此不存在"跳了却没有检查点"的丢数据窗口。`PrevBytePos` 取旧的 end-of-WAL 而非落点本身：跳跃后的第一条记录被恢复以随机访问方式读到，`ValidXLogRecordHeader` 校验 `xl_prev < RecPtr`，取落点会判负。**跳过的段号成为永久空洞**：本地崩溃恢复不受影响，原生归档/PITR 在此断链（见下节裁定） |
...
- **(a) 归档连续性**：跳过的段号永远不会产生文件，`archive_command` 也就不会为
  它们归档。后果**不是**"归档变慢"而是**PITR 在升主处断链** —— 从跳跃之前的
  基础备份出发，恢复只能走到旧段末尾。**裁定：接受，并把它写成运维规程** ——
  每次升主之后必须以该时刻为界重取基础备份。理由：本方案的高可用不建立在
  原生归档上（§10 已裁定"分片表原生流复制热备读：本来就不用，本项目用自己的
  回放"），而正确性上没有第二条路 —— 位点不推进，升主后写入的数据会在一次
  本地崩溃后被 `lsn <= PageGetLSN` 静默跳过。**用可恢复的运维代价换不可恢复的
  数据损失，是划算的。**
- **(b) `max_wal_size` / `wal_keep_size` 账目**：两者都按 LSN 距离算，一次大跳跃
  会让 checkpoint 触发逻辑瞬间失真。**裁定：无需额外处置** —— 跳跃**本身就以
  一次强制检查点收尾**（这是零窗口设计的副产品），账目在跳跃落地的同一刻即被
  重置，失真窗口长度为零。
- **(c) 该节点自带的物理备库**：会在位点跳跃处断流，须重建。**裁定：本方案不支持
  在参与 raft 分区组的节点上挂原生物理备库** —— 与 (a) 同源，且 §10 已有相同
  口径。若确有此需求，升主后重建备库。

**未闭**：这三项都是"接受并规定动作"，不是"消除"。真要消除，得让位点不跳 ——
另一条路是升主时对该分片全量重发 FPI（页 LSN 就地改写成本地坐标，复用 T6.1 的
`ShardBaselineEmit` 机件，无内核改动、无空洞），代价是 O(分片大小) 的 WAL 与
I/O **落在切主 SLA 上**。本期选了 O(1) 的位点跳跃；若将来运维面无法接受空洞，
这条替代路径是现成的。
