...
1. **3 条真开着的**：
   ...
   - `dtx_tso_p4` 的 **M5 / S1** —— 两条都报"实际取不到值"，即**度量本身没跑出
     结果**，而不是判据判负。M5 的形状值得记：它前一条
...
22. **R-P6-13 门禁仍有跨套件干扰：同一二进制上"批次红、单跑绿"**
    ...
    **危险在于它是双向的**：它既会把好的判成红（本期三例），也可能把红的盖成绿
    —— 后者更糟，因为没人会去查一条通过的断言。
    **未闭**：净场层现在只做"复位 + 拉起"，没有"**等到组稳定**"这一步。
    真修法是在 `scrub()` 末尾加一道收敛等待（各组 leader 就位且
    `partition_map` 与 `pg_dist_shard` 一致才放行下一套）。
...
21. **R-P6-12 扩展 SQL 的声明面与运行集群的实装面有 10 个函数的差集**
    ...
    **未闭**：要么给扩展加版本升级脚本（`pg_partdist--1.0--1.1.sql`）并走
    `ALTER EXTENSION UPDATE`，要么在门禁净场层重放一遍
    `CREATE OR REPLACE FUNCTION`。本期只做定性，不动交付面。
