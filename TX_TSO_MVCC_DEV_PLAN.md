...
| 编号 | 内容 | 归属 | 状态 |
|---|---|---|---|
| U-P5-1 | **follower 侧的分片 clog 未由流重建 + 无发号水位** | P6（切主/恢复全链路） | **◐ 两半都已做（2026-08-21，§3.7）**：① MARKER 携带本分区分片 xid，follower 据此重建 `pg_shard_clog`；② MARKER 再携带 leader 的**已持久化发号水位**，follower 落进自己的 `pg_shard_xid`（实测 4099，且跨 follower 重启仍在）。**残留归 P6**：水位只能新到最后一条入流的 MARKER，滴水不漏要靠 §6.6 的升主认领 —— 而认领的输入正是这个水位，两件事配套 |
