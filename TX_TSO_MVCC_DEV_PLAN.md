...
**实证（五连跑 ⑦）**：修复前 Q3 跑满 60 秒不收敛（历轮 2/3、1/3 命中失败）；
修复后 r1/r2/r3 均 **`ok:1s`** —— 一秒即学到判决，量级改善。

**但 r4 仍然失败（60 秒取不到值）——本修复未消灭 Q3，只是大幅收窄。**
不可把"1 秒收敛"当作 Q3 已闭。残留形态与失败轮的现场：r4 的 M4 切主落在
`peek@5435`，`Q3 前置：决议写入` 仍 PASS（决议确已写下），说明卡点仍在
"参与方学到判决"这一段，而非决议本身。**待加装 Q3 专项取证**（问询逐成员
返回值、pending 登记态、该 xid 的分片 clog 状态、读者 start_ts vs
commit_ts）后再定根因，**不臆测**。
