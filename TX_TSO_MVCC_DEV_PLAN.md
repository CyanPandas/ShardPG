...
- **R-P4-12 已修（2026-08-15 解冻批次 #4）**：两版修法——① `dtx_peek` 读表
  ...
  **周期性自愈**。实证：M4 由必红转为多轮绿，Q3 的 `dtx_note_coord` 漏步
  ...
  永不收敛）。
- **★ R-P4-13（M4 残余，2026-08-15 定位到此为止）**：M4 失败时新 leader 的
   日志游标与其他成员**完全一致**（12/12/12 全 applied），却**没有本地决议
   行** —— 排除追平时延（这正是 R-P4-12 修掉的那类），指向 apply 侧未走
   DTX 登记分支：`data_apply_dtx_one` 依赖 `group_local_partition()` 取
   local_oid，且 `data_apply_advance` 在 `local_oid<=0` 时直接返回 false；
   但该节点是分片本体承载者、身份映射应当存在，日志里也无任何 DECISION
   登记痕迹（连失败告警都没有）。**下一步需在 apply 侧加诊断才能定论**
   （属 pg_raft 深处，且已超出本次解冻批次 #4 的修复面）。当前影响：
   崩后 in-doubt 收敛依赖其他存活成员应答（决议在多数派上，正确性不受损）。
