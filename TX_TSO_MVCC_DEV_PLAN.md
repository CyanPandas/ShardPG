...
**★ P7 期增补（2026-09-09 用户裁定，与上面三条同等效力）**：

4. **产出边界改到 `~/shardpg-test-work` ↔ 分支 `shardpg-test`**（容器
   `pg-test-container`）。`shardpg-TX2` 与 `~/shardpg-demo` 不再接受本线改动；
   工作区装了 `pre-push` 守卫，推不到别的分支。见根目录 `PG_TEST_ENV.md`。
5. **`pg-raft-src` 解冻**：P7 期"该改就改"，**不必逐次申请**。代价是它是全簇共识层
   ——改共享结构体必须 clean rebuild（R-P6-7 的根因）、每次至少跑一轮切主套件、
   改完在 `raft_module_revision_plan.md` 留一行状态。
6. **改完就回填文档，不许攒**：每条缺陷修完的收尾动作必须包含"改哪份文档的哪一段"，
   与代码同一个提交。依据是本次审计的实证 —— 10 处不对齐里有两处是批次 #10
   自己做完没回填，一个月后谁也分不清哪句是现状。对应表见
   `pg-partdist-src/docs/P7_REMEDIATION_PLAN.md` §6.1。
7. **接口必须真的可用，不许假声明**：新加/改动的任何 SQL 可见接口，**必须在真集群上
   被调用成功过一次**，并在验收脚本里留下断言。只写 `CREATE FUNCTION`、只改头文件、
   只在设计文档里写签名，都不算做完。本项目已踩出三种"假可用"：声明了但库里没有
   （R-P6-12）、函数在但调用方少写模式名（R-P6-20）、**守卫在但生产形态下不生效**
   （R-P6-22）。详见 P7 计划 §6.2。
...
#### 批次 #12：全项目文档回填 + P7 收口计划（2026-09-09）

**不改任何产品代码**，只做两件事：把文档拉回与代码一致，和把"接下来怎么补"写死。

**环境切换**：本批次起，产出落在**新的隔离场地** —— 容器 `pg-test-container`
（1 coordinator + 8 worker）、工作区 `~/shardpg-test-work`、GitHub 分支
`shardpg-test`（从 `shardpg-TX2` 尖端 `efc6a2e` 原样分出）。工作区装了 `pre-push`
守卫，**推不到 `shardpg-TX2` 以外的任何分支**，`shardpg-TX2` 与演示材料因此不受
后续改动干扰。建法与切换规矩见根目录 `PG_TEST_ENV.md`。

**回填的 10 处**（审计 `P6_EXIT_AUDIT.md` §四逐处复核后改）：DESIGN 头部状态、
DESIGN §1.1（Proxy 正式注销）、DESIGN §10（补 4 行 / 改 2 行）、FRD §10 签名、
FRD §11 步骤 5、FRD §13 约束 13 状态注、FRD §14.2（R3/R4 阻断解除）、
raft plan §0.2（11 行逐行复核）、DEV PLAN"不做的事"表 + P6 出口清单、
`P6_PRECHECK.md` R-P4-20 收口、根 `README.md` 导读。明细见
`pg-partdist-src/docs/P7_REMEDIATION_PLAN.md` §5。

**★ 回填过程中被代码推翻的两处**（审计当时记的状态本身已过时，如实留痕）：

1. FRD §13 约束 13 的状态注写着"pg_raft 侧修法已整体回退，当前代码 = 稳定版 +
   isnull 修复"——**错**。T4.4 已按用户裁定 (b) 重放 #39，09-09 实测
   `wait_for_log_room` 4 处、`apply_owner_pid` 11 处、`flow_stats` 13 处**都在库**。
   连带一条：测试侧 `health_check_no_drops` 的"`flow_stats` 不存在时打跳过"也已过时，
   它现在**真正生效**——这意味着此前"全 PASS"的轮次里，丢弃断言是不是真跑过需要复核。
2. DEV PLAN"不做的事"表里"分片 clog 物理删除"记的是"只登记不删"——**已做**
   （`shard_clog.c:392-412` 提交时 `rmtree`）。

**新登记 R-P6-15 ~ R-P6-21**（§5 台账）：多分片演示实跑撞出的七条，此前只躺在
...
##### 用户裁定（2026-09-09，六条，已写进 P7 计划 §3）

1. **批次 6 做**，排在批次 1–5 之后；**`pg-raft-src` 解冻，该改就改，不必再申请**。
2. **引用表运行期写：拦**（守卫按 T7.10 实装；DESIGN §10 该行已改）。
3. **OPS 8 套：改造进门禁**（拓扑无关化），门禁口径 31 → **39 套**。
4. **`replica_gate_p6`：接受 T6.3c 改写后的命题**，不补真实回卷压力；出口清单该条
   改判 ✅（12 条现为 ✅7 / ◐3 / ❌2）。
5. **全量暂时不跑**。
6. **`citus_drain_node` 那条先搁置**，只要求解释清楚 —— 解释过程中**撞出 R-P6-22**
   （见 §5 台账第 26 条），比原问题严重得多：那道守卫在协调者上根本不生效。
...
| T7.29 | P7-G4 commit_ts 双宇宙无标志位（中途配 TSO 已提交行消失） | `cts_universe_p7` 22/0（修前 21/1：40 行→0 行）；回归 11 套批次红单跑复核全绿，`txn_layer_r2` 51/1 经 A/B 判为先前就有（P7-T8） | 本次 |
...
> **★ 2026-09-09 回填**：下表原表头是"建议去向（**待裁**）"，九行至今无一有裁定
> 记录（出口清单第 11 条因此挂着）。本次补上「实况（09-09 复核代码）」列，并把
> 处置计划落到 `pg-partdist-src/docs/P7_REMEDIATION_PLAN.md`；**裁定本身仍待用户
> 拍板**，见该文 §3。

| 来源 | 事项 | 建议去向（待裁） | **实况（09-09 复核）** |
|---|---|---|---|
| P5 挂账 ③ | 分片 vacuum 的**自动启动器**没做（今天只能手工调） | 做（否则生产不可用） | **未做**：到龄只 WARNING（`shard_xid.c:1381`）。P7 计划 T7.17 |
| P5 挂账 ④ | **回卷本身未实现**（全仓用普通 `<`/`>=`，无模运算） | 降级为已知边界：阶段 2 停发线让线性假设成立，写进 §10 | **未做**，且 §10 一直没这一行 —— **本次已补进 DESIGN §10**（降级为已知边界：线性 xid + 停发线） |
| P5 挂账 ⑥ | 两处覆盖缺口（clog **整段删除**分支；真正停在**页面循环中间**的崩溃） | 做（后者需在生产路径加故障注入点） | **未做**：需在生产路径加故障注入点。P7 计划 T7.19 |
| P5 挂账 ⑦ | 分片 vacuum 缺**尾部截断** | 做（V4 复评已认定它是 VACUUM FULL 在 P5 之后唯一还能提供的东西，且不需要 rewriteheap） | **未做**：`shard_vacuum.c` 无 `smgrtruncate`/`RelationTruncate`。P7 计划 T7.18 |
| T5.8 收窄项 | **LP_REDIRECT 未实现**，与"解禁用户索引"同一件事 | 随索引专项；P6 若不解禁索引则写进 §10 | **未做**，撞见即 ERROR（`shard_vacuum.c:715-736`）—— **本次已连同"打标后禁 CREATE INDEX"补进 DESIGN §10** |
| §13 约束 13 | Raft 日志环 `RAFT_LOG_CAPACITY=128`，背压超时（默认 10s）后**仍然丢弃** ⇒ 永久分叉 | 做（提容量 + 丢弃留痕；`flow_stats` 观测在流控回退时没跟着回来） | **部分做**：检测（`diverged` 标记）+ 一键修复（`repair_diverged_shards`）已由批次 #8/#9 落地；**容量仍是 128、未提，也无自动修复**；`flow_stats` 观测已随 T4.4 重放回库（09-09 实测在库） |
| §3.1 两次推迟 | 分片 clog 的**物理删除**（`ShardClogRememberDrop` 只登记不删） | 做或裁定 | ✅ **已做**（表陈旧）：`ShardClogAtCommit` 提交时 `rmtree`（`shard_clog.c:392-412`，09-09 实测命中）。**本行可以划掉** |
| §3.3 两次推迟 | 独立 **Proxy 守护组件** | 文档原话："若 P6 仍无消费方应考虑**从方案中裁掉**" | ✅ **已裁掉**：实质由 Citus MX 取代（T4.2），**本次已在 DESIGN §1.1 正式注销** |
| §14.2 | promoted shard 的可读性**硬阻断于 R3** | 裁定 R3 是否进 P6 范围 | **已不成立（新宇宙）**：打标分片走分片 xid + 分片 clog + TSO 快照，promoted 可读已由 tx3/p7 实证；R3 四个符号从未实装、只剩遗留宇宙需要 ⇒ 建议裁定"遗留副本退役"。**本次已在 FRD §14.2 回填** |

---

#### P6 出口清单（全勾即结项）

> **★ 2026-09-09 逐条核验（`P6_EXIT_AUDIT.md` §二 + 本次复核）：✅ 7 / ◐ 3 / ❌ 2。
> P6 没有结项，且两条 ❌ 都不是"功能没做"，是出口动作本身没做。**
> 收口计划见 `pg-partdist-src/docs/P7_REMEDIATION_PLAN.md`。

- [x] **✅ T6.0 前置核查落档，风险登记簿无陈旧状态** —— 落档 `P6_PRECHECK.md`；
      R-P4-20 的"★ 未修"与 FRD §13 约束 12 的"已处置完毕"两处矛盾**本次已收口**；
      R-P4-13 保持"已绕行、根因未闭"（这是准确状态，不是陈旧）
- [ ] **◐ 物理基线重做工具可用，三个消费者各有一条端到端用例** —— 工具 ✓；
      初始配对 ✓（`shard_baseline_p6`/`handover_provision_p7`）、永久分叉 ✓
      （`divergence_mark_p8`）、**快路径分叉 ✗**（`fastpath_divergence_tx4` 只验到
      `promote_prepare` 返回 -1，无重做基线步骤）。P7 计划 T7.14
- [x] **✅ 无基线游标的 locmap 配对被拒绝**（`locmap_base_p6` 25/0）
- [x] **✅ 注入"页太短"⇒ shard 停摆、节点不重置**（`offnum_guard_p6` 25/0）
- [x] **✅ 升主后写入 + 本地崩溃恢复不丢**（`promote_p6` 39/0，含 kill -9 取证）
- [x] **✅（2026-09-09 裁定后改判）副本壳表不被本地 WAL 触碰** —— 用户裁定
      **接受 T6.3c 改写后的命题**：判据是"约束 5 relfrozenxid 已同步 + 副本读闸门
      拦截"（`replica_gate_p6` 29/0），**不再要求施加真实 anti-wraparound 压力**
      （前提不成立故收缩，改写后的命题逻辑上更强）。本条原文即以此为准。
      遗留残留仍记账：遗留宇宙副本的读会设 hint bit（本地写），随"遗留副本退役"处理
- [x] **✅ 升主后无主 RUNNING 全部落定；U-P5-1 窗口被认领兜住**（`promote_p6` 含三条阴性）
- [x] **✅ 回放推进分片分配器**（`xid_watermark_p6` 17/0；批次 #11 后发号起点收成
      旧主真实 next_xid）
- [ ] **◐ §10 每条限制各有一条负向断言** —— 缺口两处：**引用表运行期写代码零守卫、
      测试零断言**；`citus_drain_node` 这类不经 ExecutorStart 的**加名字也挡不住**。
      §10 本身缺的 4 行、错的 1 行**本次已回填**。P7 计划 T7.9/T7.10
- [ ] **❌ 全量一次跑完、零 FAIL** —— 口径应为 **31 套**（`run_p6_exit.sh` `SUITES` 实数；
      原文"29 套"过时；OPS 8 套按 R-P6-1 第二次修订默认不进，去向待裁）。最近一次
      全量是 T6.8 的 28 套 1191/31，**其后批次 #7–#11 改了回放 redo 路径、路由层、
      `promote_prepare`、整个 PG 二进制、发号起点 —— 31 套在最新二进制上一次没跑过**。
      最坏 9.5 h。P7 计划 T7.12
- [ ] **◐ 上表"不做的事"逐条有裁定** —— 九行实况已逐行填（见上表新增列），去向也已定：
      **裁掉 2**（Proxy 守护组件、R3 可读性/遗留副本退役）、**划掉 1**（分片 clog 物理
      删除，已做）、**接受并已写进 DESIGN §10 共 2**（回卷、LP_REDIRECT + 打标后禁
      CREATE INDEX）、**做 3**（vacuum 自动启动器 / 尾部截断 / 两处覆盖缺口 → P7 批次 5）、
      **归批次 6 共 1**（§13 约束 13 环容量：检测 + 手工修复为当前形态，提容量随
      raft 专项）。**仍开口的是"接受"类是否要单独出一份运维规程** —— 定了就能勾
- [x] **✅（本次完成）三份设计文档与代码逐条对齐** —— 审计 §四列的 10 处不对齐
      2026-09-09 已逐处回填，清单见 `P7_REMEDIATION_PLAN.md` §5。**后续每改一处代码
      同步回填**，否则立刻退化
...
24. **R-P6-14 逻辑解码禁令只覆盖 SQL 调用，walsender 路径绕得过去**
    ...
    **→ 2026-09-13 已闭（T7.22，未动内核）**：复制协议上的逻辑解码只有
    `replication=database` 一个入口 —— 认证钩子在那里拒；已连着的在打标登记时
    SIGTERM；"写入时打标、解码前撤标"这条窄绕法由持久化的逻辑解码围栏堵
    （最后一张打标表 DROP 时的位点，确认位点早于它的逻辑槽一律拒）。
    测试通道的 GUC 白名单被撤下时不立围栏，不覆盖。详见 P7_REMEDIATION_PLAN R-P6-14。
...
25. **R-P6-15 ~ R-P6-21：多分片演示实跑撞出的七条（2026-09-06/07 实测，2026-09-09
    正式登记）**。此前它们只存在于 `P6_PENDING_DECISIONS.md` 的附录里（当时按"只写
    演示文档"的指示撤回），**没有进台账 = 没有人会去修**。本次补登记，逐条修法与
    验收见 `pg-partdist-src/docs/P7_REMEDIATION_PLAN.md`。
...
26. **R-P6-22 §9.2 第 3 层禁用清单在协调者上根本不生效**（2026-09-09 查证第 6 条
    ...
    （`shard_xid.c:248`）。**分片打标表长在 worker 上；协调者既没有打标表、
    也没人给它设白名单**——实测本环境三个节点 `SHOW pg_partdist.shard_relids` 全空。

    ⇒ `citus_rebalance_start` / `citus_drain_node` / `undistribute_table` /
    `alter_distributed_table` 这些**恰恰只在协调者上调用**的 UDF，禁令在生产形态下
    **一条都不会触发**。§9.2 第 3 层整层熄火。

    **`negative_p6` 为什么全绿**：夹具替它把闸门打开了（`test_negative_p6.sh:64-66`），
    而且用的是 **worker 5433 上那张本地表 `t66neg` 的 OID** —— 在协调者上它不指向任何
    打标表。脚本注释当时就写了"白名单只设在 worker 上时，协调者那支根本不生效
    （首版因此取空）"，**但没人意识到这句注释同时证明了生产形态下这道守卫是熄火的**。

    这是"假可用"的第三种形态（前两种是 R-P6-12 声明了库里没有、R-P6-20 名字拼不全）：
    **守卫存在、测试全绿、生产不生效**。

    **修法**（P7 T7.9，与补 8 个 UDF 名字同批，且**必须先做这条**）：Citus 运维类
    禁令的闸门不能用 `ShardGatingActive()`（那是"本节点有没有打标表"），要改成
