...
31 套全量要留到 P7 批次 1–3 改完之后跑，现在跑出来的数字改完就作废（P7 §3 第 5 条）。
...
> **★ 2026-09-09 逐条核验（`P6_EXIT_AUDIT.md` §二 + 本次复核）：✅ 6 / ◐ 4 / ❌ 2。
...
- [ ] **◐ 副本壳表在 anti-wraparound 压力下不被本地 WAL 触碰** —— 套件
      （`replica_gate_p6`）**没有施加任何回卷压力**，验的是"relfrozenxid 已同步 +
      闸门拦截"。T6.3c 把命题改写了（前提不成立故收缩），逻辑成立但**清单原文的
      实验没做**；另有已记残留：遗留宇宙副本的读会设 hint bit。P7 计划 T7.15（须裁定）
...
- [ ] **❌ 上表"不做的事"逐条有裁定** —— 九行**实况本次已逐行填**（见上表新增列），
      但**裁定仍待用户拍板**（P7 计划 §3）
