...
- [ ] 单分片读写混合 + 全崩溃点可见性正确（提交者崩后可见、未决者崩后不可见）
- [ ] 无主 RUNNING 认领生效（里程碑门禁原文）
- [ ] 临时提交表代码退役删除（风险登记簿 4"临时桩滞留"清偿）
- [ ] ANALYZE 解禁；V4 裁定落档设计 §11
- [ ] 四笔 P1 债清偿（DROP GC / partition_map 门控 / WAL 恢复推进 / xid_map
      路径隔离判定落档）
- [ ] 全量 438 + P1 两套件 + P2 新套件零新增 FAIL
- [ ] patches/README 更新 + pg-install 同步提交成对完成（0007）
