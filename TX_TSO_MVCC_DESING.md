...
> 一条都不认领。
>
> **区间上界 = 交接来的 `next_xid`，不再是宽一个批次的持久化水位**（2026-09-07
> 更正，批次 #11）。此前 MARKER 带的是按 `SHARD_XID_BATCH`=4096 向上取整的持久化
> 水位，理由是"中止且字节未入流的事务会悄悄吃掉号，宽一个批次正好盖住"；实测代价
> 是切主后新主从 4099 起发号，而旧主明明只发到 9 —— 违背"号在分片上、主换了号不断"。
> 那段窗口不需要靠跳号来盖：字节**入了流**的号 follower 都看得见（MARKER 带分片
> xid；DATA 记录里的分片 xid 由回放侧喂进影子水位），升主时与交接值取 Max，绝不
> 重发；字节**没入流**的号在旧主降级/重做基线后不存在于本分片宇宙里，重发无害。
> 于是新主从旧主真实的 `next_xid` 继续，认领只把"发出去了、流里却没有提交标记"的号
> 改判 ABORTED。leader 自己的批次水位只用于本机崩溃恢复，不交接。
> 实测（`DEMO_WINDOWS_POWERSHELL.md` 第 10 步）：旧主下个号 10，切主后新主
> `shard_xid_next = 10`、首行分片 xid = 10，被 ROLLBACK 的 9 判 ABORTED，4099 空槽。
>
...
| **中途启用 TSO**（先以遗留模式写数据，之后才配 `tso_conninfo`） | **可见性已修（T7.29，2026-09-13）；vacuum 截断仍受限——★ 2026-09-13 补行，已知边界** | 可见性：commit_ts 是双宇宙字段，MARKER 现带 `CTS_IS_TSO` 位、gclog 槽记宇宙位，非 TSO 宇宙的 commit_ts 按 0（对一切快照可见）比较，已提交行不再"消失"（`test_cts_universe_p7.sh`）。截断：§6.3 前缀扫描把"COMMITTED 且 commit_ts=0"判为 commit-ts-too-new 停下，遗留模式留下的提交会**挡住 clog 截断前缀**（少清不错清，但龄会涨向停发线）。放宽需先逐路径证明"TSO 模式提交永不存 0"，截断不可逆，故暂不放宽；运维上应在写入数据之前配好 TSO |
