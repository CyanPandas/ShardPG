...
| 引用表运行期写入 | **禁（2026-09-09 裁定：拦）**——★ 复核发现代码**零守卫、测试零断言**（`shard_guard.c` 全文无引用表判据），此前本行只是纸面约定。守卫按裁定实装中（`P7_REMEDIATION_PLAN.md` T7.10：按 `pg_dist_partition.partmethod='n'` 判引用表并拒写），**在守卫落地之前本行拦不住，靠规程** | 无分片写集 ⇒ 无协调者分片；Citus 原生 2PC 恢复已关（§9.2） |
